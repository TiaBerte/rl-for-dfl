...
from usecases.wsmc.solve_instances import MinSetCoverProblem, compute_cost
...
class MinSetCoverEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.action_space = Box(low=0, high=1000, shape=(self._num_prods, ), dtype=np.float32)
        ...
        self._demands_scaler.fit(train_demands)
    ...
    def _load_instances(self) -> Tuple[List[MinSetCover], np.ndarray, List[float]]:
        ...
        # Each instance is in a separated folder
        inst_dirs = os.listdir(self._instances_filepath)

        for f in tqdm(inst_dirs,
                      total=len(inst_dirs),
                      desc='Gym environment - Loading instances'):
            ...
            if os.path.isdir(path):
                ...
                # Load the instance and the optimal cost
                instance = load_msc(instance_path)
                instances.append(instance)

                demands.append(instance.demands)

                cost_file = open(optimal_cost_path, 'rb')
...
def generate_wsmc_instances(num_instances: int,
                            num_sets: int,
                            num_products: int,
                            density: float,
    ...
    # List of linear function that describe how the poisson rates depend on the observables
    fun = list()
...
def generate_training_and_test_sets(data_path: str,
    ...
    instances = \
        generate_wsmc_instances(num_instances=num_instances,
                                num_sets=num_sets,
    ...
    for idx, inst in tqdm(enumerate(instances),
        ...
        with open(os.path.join(instance_path, f'optimal-sol.pkl'), 'wb') as file:
            pickle.dump(optimal_sol, file)
        with open(os.path.join(instance_path, f'optimal-cost.pkl'), 'wb') as file:
            pickle.dump(optimal_cost, file)
